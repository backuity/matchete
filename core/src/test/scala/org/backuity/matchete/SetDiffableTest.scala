...
class SetDiffableTest extends JunitMatchers {
  ...
  def diffMustShowMissingElements(): Unit = {
    ...
    {
      Set(Person("john", 12), Person("doe", 23)) must_== Set(Person("doe", 99), Person("john", 12))
    } must throwAn[AssertionError].withMessage(
      """Set(john(12), doe(23)) is not equal to Set(doe(99), john(12))
        |Got     : Person(doe,..).age = 23
        |Expected: Person(doe,..).age = 99""".stripMargin)

    {
      Set(Person("john", 12), Person("doe", 23), Person("xx", 45)) must_== Set(Person("doe", 99), Person("john", 12))
    } must throwAn[AssertionError].withMessage(
...

    {
      ...
      """
        ...
        |
        |Got     : (1).Person(doe,..).age = 23
        |Expected: (1).Person(doe,..).age = 99""".stripMargin)
  }
}
